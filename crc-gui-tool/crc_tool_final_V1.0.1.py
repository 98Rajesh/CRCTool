...
class CRCTool(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Universal CRC & HEX Tool")
    ...
    def build_header(self, parent):
        ...
        try:
            img = Image.open("crc_icon.png")
            img = img.resize((48, 48))
            self.logo = ImageTk.PhotoImage(img)
            ttk.Label(header, image=self.logo).pack(side=tk.LEFT, padx=10)
        except Exception:
            ttk.Label(header, text="CRC",
                    font=("Segoe UI", 24, "bold")).pack(side=tk.LEFT, padx=10)
